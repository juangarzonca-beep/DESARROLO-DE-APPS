...
var keyboard = require('./keyboard-C06ZtyfO.js');
require('./index-DLQNazpt.js');
...
const enableScrollAssist = (componentEl, inputEl, contentEl, footerEl, keyboardHeight, enableScrollPadding, keyboardResize, disableClonedInput = false) => {
     ...
     */
    const platformHeight = helpers.win !== undefined ? helpers.win.innerHeight : 0;
    /**
    ...
    const focusOut = () => {
        ...
        helpers.win?.removeEventListener('ionKeyboardDidShow', keyboardShow);
    ...
    const focusIn = async () => {
        ...
        helpers.win?.addEventListener('ionKeyboardDidShow', keyboardShow);
...
const jsSetFocus = async (componentEl, inputEl, contentEl, footerEl, keyboardHeight, enableScrollPadding, disableClonedInput = false, platformHeight = 0, waitForResize = true) => {
    ...
    if (typeof window !== 'undefined') {
        ...
        const scrollContent = async () => {
            ...
            if (contentEl) {
                await index.scrollByPoint(contentEl, 0, scrollData.scrollAmount, scrollData.scrollDuration);
            }
        ...
        if (contentEl) {
            const scrollEl = await index.getScrollElement(contentEl);
            /**
...
const startInputShims = async (config, platform) => {
    ...
    const inputs = Array.from(helpers.doc.querySelectorAll('ion-input, ion-textarea'));
    ...
    const registerInput = async (componentEl) => {
        ...
        const scrollEl = index.findClosestIonContent(componentEl);
    ...
    helpers.doc.addEventListener('ionInputDidLoad', (ev) => {
        registerInput(ev.detail);
    });
    helpers.doc.addEventListener('ionInputDidUnload', (ev) => {
