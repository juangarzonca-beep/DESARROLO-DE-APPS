...
var index$2 = require('./index-DLQNazpt.js');
var index = require('./index-m83Rr85o.js');
var frameworkDelegate = require('./framework-delegate-CvJ6SkeA.js');
var helpers = require('./helpers-CJUjGJOS.js');
var lockController = require('./lock-controller-aDB9wrEf.js');
var capacitor = require('./capacitor-CtkuLHaA.js');
var hardwareBackButton = require('./overlays-BEVysBKh.js');
var theme = require('./theme-Bw0rpaNN.js');
var index$3 = require('./index-78cPCZwH.js');
var ionicGlobal = require('./ionic-global-t_WpJo84.js');
var keyboard = require('./keyboard-CrDGk8Si.js');
var animation = require('./animation-C-JWwKgy.js');
var cubicBezier = require('./cubic-bezier-DAjy1V-e.js');
var index$1 = require('./index-DRPqwSvU.js');
require('./dir-zwDPRE5K.js');
require('./gesture-controller-CeJslptU.js');
require('./keyboard-C06ZtyfO.js');
...
const setCardStatusBarDark = () => {
    if (!helpers.win || helpers.win.innerWidth >= 768) {
        return;
    }
 ...
 */
const onModalHeightChange = (hostEl, callback) => {
    return helpers.onCustomPropertyChange(hostEl, '--height', () => callback());
};
...
const canSwipeOnContent = (contentEl) => {
    const scrollEl = index.isIonContent(contentEl) ? helpers.getElementRoot(contentEl).querySelector('.inner-scroll') : contentEl;
    const hasRefresherInContent = !!index.findRefresherInContent(contentEl);
...
const handleCanDismiss = async (el, animation) => {
     ...
     */
    const shouldDismiss = await el.canDismiss(undefined, hardwareBackButton.GESTURE);
    if (!shouldDismiss) {
        return;
    }
    /**
...
const createSwipeToCloseGesture = (el, animation, statusBarStyle, onDismiss, onDragStart, onDragMove, onDragEnd) => {
    ...
    let lastStep = 0;
    const getScrollY = () => {
        if (contentEl && index.isIonContent(contentEl)) {
    ...
    const canStart = (detail) => {
         ...
         */
        contentEl = index.findClosestIonContent(target);
        if (contentEl) {
    ...
    const onStart = (detail) => {
        ...
        if (deltaY > 0 && contentEl) {
            index.disableContentScrollY(contentEl);
        }
    ...
    const onEnd = (detail) => {
        ...
        if (contentEl) {
            index.resetContentScrollY(contentEl, initialScrollY);
        }
        animation
            .onFinish(() => {
            if (!shouldComplete) {
    ...
    const gesture = index$1.createGesture({
        ...
        gesturePriority: hardwareBackButton.OVERLAY_GESTURE_PRIORITY,
        direction: 'y',
        threshold: 10,
        canStart,
        onStart,
...
const createSheetGesture = (baseEl, backdropEl, wrapperEl, initialBreakpoint, backdropBreakpoint, animation, breakpoints = [], expandToScroll, getCurrentBreakpoint, onDismiss, onBreakpointChange, onDragStart, onDragMove, onDragEnd) => {
    ...
    const enableBackdrop = () => {
         ...
         * the sheet.
         */
        baseEl.classList.remove(hardwareBackButton.FOCUS_TRAP_DISABLE_CLASS);
    };
    const disableBackdrop = () => {
         ...
         */
        baseEl.classList.add(hardwareBackButton.FOCUS_TRAP_DISABLE_CLASS);
    };
    /**
    ...
    const canStart = (detail) => {
         ...
         */
        const contentEl = index.findClosestIonContent(detail.event.target);
    ...
    const onStart = (detail) => {
         ...
         */
        if (!expandToScroll) {
            const targetEl = index.findClosestIonContent(detail.event.target);
            cachedScrollEl =
                targetEl && index.isIonContent(targetEl) ? helpers.getElementRoot(targetEl).querySelector('.inner-scroll') : targetEl;
        }
        /**
...
const getRootSafeAreaTop = () => {
    ...
    const doc = helpers.win?.document;
    if (!doc?.body) {
        return 0;
    }
...
const onRootSafeAreaTopChange = (callback) => {
    return helpers.onCustomPropertyChange(helpers.win?.document?.body, '--ion-safe-area-top', callback, getRootSafeAreaTop());
};
/**
 ...
 */
const getModalCoveredAxes = (hostEl) => {
    const styles = getComputedStyle(hostEl);
    const width = hardwareBackButton.getOverlaySizeType(styles.getPropertyValue('--width'));
    const height = hardwareBackButton.getOverlaySizeType(styles.getPropertyValue('--height'));
 ...
 */
const measureCoveredAxes = (hostEl) => {
    const wrapperEl = hostEl.shadowRoot?.querySelector('.modal-wrapper');
    if (wrapperEl == null || helpers.win === undefined) {
    ...
    return {
        vertical: height >= helpers.win.innerHeight - EDGE_THRESHOLD,
        horizontal: width >= helpers.win.innerWidth - EDGE_THRESHOLD,
    };
};
/**
...
const getPositionBasedSafeAreaConfig = (wrapperEl) => {
    ...
    const vh = helpers.win?.innerHeight ?? 0;
    const vw = helpers.win?.innerWidth ?? 0;
...
const Modal = class {
    ...
    connectedCallback() {
        const { el } = this;
        hardwareBackButton.prepareOverlay(el);
        this.triggerChanged();
        ...
        if (this.presented) {
            hardwareBackButton.restoreRootFocusTrapAccessibility(el);
    ...
    async present() {
        ...
        try {
            ...
            await hardwareBackButton.present(this, 'modalEnter', iosEnterAnimation, mdEnterAnimation, {
    ...
    initSwipeToClose() {
        ...
        const contentEl = index.findIonContent(el);
        if (!contentEl) {
            index.printIonContentErrorMsg(el);
            return;
        }
    ...
    sheetOnDismiss() {
        ...
        this.animation.onFinish(async () => {
            ...
            await this.dismiss(undefined, hardwareBackButton.GESTURE);
            this.gestureAnimationDismissing = false;
        });
    }
    cardOnDismiss() {
    ...
    async dismiss(data, role) {
        if (this.gestureAnimationDismissing && role !== hardwareBackButton.GESTURE) {
            return false;
        }
        ...
        const dismissed = await hardwareBackButton.dismiss(this, data, role, 'modalLeave', iosLeaveAnimation, mdLeaveAnimation, {
            presentingEl: presentingElement,
     ...
     */
    onDidDismiss() {
        return hardwareBackButton.eventMethod(this.el, 'ionModalDidDismiss');
    }
    /**
     * Returns a promise that resolves when the modal will dismiss.
     */
    onWillDismiss() {
        return hardwareBackButton.eventMethod(this.el, 'ionModalWillDismiss');
    }
    /**
    ...
    render() {
        ...
        return (index$2.h(index$2.Host, { key: '17a95bd7722bac2344bea041aff97db17f55c0cf', "no-router": true,
            ...
            tabIndex: isHandleCycle && isSheetModalWithHandle ? 0 : -1, ...htmlAttributes, style: {
                ...
                'overlay-hidden': true,
                [hardwareBackButton.FOCUS_TRAP_DISABLE_CLASS]: focusTrap === false,
                ...theme.getClassMap(this.cssClass),
            }, onIonBackdropTap: this.onBackdropTap, onIonModalDidPresent: this.onLifecycle, onIonModalWillPresent: this.onLifecycle, onIonModalWillDismiss: this.onLifecycle, onIonModalDidDismiss: this.onLifecycle, onFocus: this.onModalFocus }, index$2.h("ion-backdrop", { key: '9dab1ef4771c0d847ba2be8672afbaa2748517f3', ref: (el) => (this.backdropEl = el), visible: this.showBackdrop, tappable: this.backdropDismiss, part: "backdrop" }), mode === 'ios' && index$2.h("div", { key: '64af7ed1c5f552b61fc4b9387b926acc76c8a99a', class: "modal-shadow" }), index$2.h("div", { key: '7e76d77d5b2e8f0467bb793facf5aa86f8d01bbe',
            /*
            ...
            */
            role: "dialog", ...inheritedAttributes, "aria-modal": "true", tabIndex: -1, class: "modal-wrapper ion-overlay-wrapper", part: "content", ref: (el) => (this.wrapperEl = el) }, showHandle && (index$2.h("button", { key: '5efc0bb0a59adcc594ac48da9fd90e57908a7ffb', class: "modal-handle",
            // Prevents the handle from receiving keyboard focus when it does not cycle
            tabIndex: !isHandleCycle ? -1 : 0, "aria-label": "Activate to adjust the size of the dialog overlaying the screen", onClick: isHandleCycle ? this.onHandleClick : undefined, part: "handle", ref: (el) => (this.dragHandleEl = el) })), index$2.h("slot", { key: '6175fcd0b39c086b4e9eb9e96c4d631c2a86774b', onSlotchange: this.onSlotChange }))));
    }
